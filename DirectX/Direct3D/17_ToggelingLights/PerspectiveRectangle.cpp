...
struct CBUFFER
{
	...
	unsigned int PerVertexLightingEnable;
...
float lightPosition[] = { 75.0,75.0,-20.0,1.0 };
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case 'v' :
			if (pervertexleLight)
				pervertexleLight = false;
			else
				pervertexleLight = true;
...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"struct perfragment_output" \
		"{" \
		"float4 position : SV_POSITION ; " \
		"float3 t_norm  : NORMAL0; " \
		"float3 light_dir   : NORMAL1; " \
		"float3 viewer_vector : NORMAL2; " \
		"float4 fong_ads_color : NORMAL3;" \

		"};" \

		"perfragment_output main(float4 pos : POSITION, float4 normal : NORMAL) " \
		"{" \
		"struct perfragment_output output;" \
		"if(pervertexLightingEnable == 1)" \
		"{" \
			...
			"float3 viewervector = normalize((float3)-eyeCoordinate.xyz);" \
	...
	const char *pixelShaderSourceCode =
		...
		"float4 main(float4 pos : SV_POSITION, perfragment_output Input) : SV_TARGET" \
		"{" \
		"if(pervertexLightingEnable == 1)"
		"{"
			"float4 color = Input.fong_ads_color;" \
			"return(color);" \
		"}" \
		"else" \
		"{" \
...
void display(void)
{
	...
	if (pervertexleLight == true)
		constantBuffer.PerVertexLightingEnable = 1;
	else
		constantBuffer.PerVertexLightingEnable = 0;
