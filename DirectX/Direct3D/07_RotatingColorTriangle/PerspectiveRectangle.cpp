...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Position_BRK = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Color_BRK = NULL;
ID3D11InputLayout *gpID3D11InputLayout_BRK = NULL;
ID3D11Buffer *gpID3D11Buffer_ConstantBuffer_BRK = NULL;

float gAnglePyramidBRK = 0.0f;
...
HRESULT initialize(void)
{
	...
	bufferDesc_Position.ByteWidth = sizeof(float) * ARRAYSIZE(vertices);
	bufferDesc_Position.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_Position.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device_BRK->CreateBuffer(&bufferDesc_Position, NULL, &gpID3D11Buffer_VertexBuffer_Position_BRK);
	...
	bufferDesc_color.ByteWidth = sizeof(float) * ARRAYSIZE(color);
	bufferDesc_color.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_color.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device_BRK->CreateBuffer(&bufferDesc_color, NULL, &gpID3D11Buffer_VertexBuffer_Color_BRK);
	...
	gpID3D11DeviceContext_BRK->Map(gpID3D11Buffer_VertexBuffer_Position_BRK, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, vertices, sizeof(vertices));
	gpID3D11DeviceContext_BRK->Unmap(gpID3D11Buffer_VertexBuffer_Position_BRK, NULL);

	gpID3D11DeviceContext_BRK->Map(gpID3D11Buffer_VertexBuffer_Color_BRK, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, vertices, sizeof(color));
	gpID3D11DeviceContext_BRK->Unmap(gpID3D11Buffer_VertexBuffer_Color_BRK, NULL);
	...
	gpID3D11Device_BRK->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);
...
void display(void)
{
	...
	gpID3D11DeviceContext_BRK->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_Position_BRK, &stride, &offset);

	gpID3D11DeviceContext_BRK->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Color_BRK, &stride, &offset);
	...
	XMMATRIX rotationMatrix = XMMatrixRotationY(-gAnglePyramidBRK);
	...
	if (gAnglePyramidBRK > 360)
		gAnglePyramidBRK = 0.0f;
	else
		gAnglePyramidBRK = gAnglePyramidBRK + 0.002f;


	//if ((gAnglePyramidBRK > 180) && (gAnglePyramidBRK < 360))
...
void unintialize(void)
{
	...
	if (gpID3D11Buffer_VertexBuffer_Position_BRK)
	{
		gpID3D11Buffer_VertexBuffer_Position_BRK->Release();
		gpID3D11Buffer_VertexBuffer_Position_BRK = NULL;
	}
