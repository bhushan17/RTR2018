...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"{" \
			...
			"{" \
				...
				"float3 S = normalize((float3)(lightPosition - eyeCoordinate));" \
	...
	bufferDesc.ByteWidth = _ARRAYSIZE(cube_vertices) * sizeof(float);
	...
	memcpy(mappedSubresource.pData, cube_vertices, _ARRAYSIZE(cube_vertices) * sizeof(float) );
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC; //Write access by CPU and GPU
	bufferDesc.ByteWidth = _ARRAYSIZE(cube_normals) * sizeof(float);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE; // Allow CPU to write into this buffer
	...
	memcpy(mappedSubresource.pData, cube_normals, _ARRAYSIZE(cube_normals) * sizeof(float));
...
void display(void)
{
	...
	XMMATRIX translationMatrix = XMMatrixTranslation(0.0f, 0.0f, 9.0f);
	...
	XMMATRIX wvMatrix = rotationMatrix * rotationMatriX * rotationMatriZ * translationMatrix *worldMatrix * viewMatrix ;//* gPerspectiveProjectionMatrix;
	...
	if (gblight == true)
	{
		...
		constantBuffer.LightPositiion = XMVectorSet(0.0, 0.0, -2.0, 1.0);
	}
	else
	{
