...
float RedlightPosition[] = { 0.0,0.0,0.0,1.0 };


float GreenLightAmbient[] = { 0.0,0.0,0.0,1.0 };
float GreenLightDiffuse[] = { 0.0,1.0,0.0,1.0 };
float GreenlightSpecular[] = { 0.0,1.0,0.0,1.0 };
float GreenlightPosition[] = { 0.0,0.0,0.0,1.0 };


float BlueLightAmbient[] = { 0.0,0.0,0.0,1.0 };
float BlueLightDiffuse[] = { 0.0,0.0,1.0,1.0 };
float BluelightSpecular[] = { 0.0,0.0,1.0,1.0 };
float BluelightPosition[] = { 0.0,0.0,0.0,1.0 };
...
float RedfirstRotation = 0.0f;
float RedsecondRotation = 0.0f;
float GreenfirstRotation = 0.0f;
float GreensecondRotation = 0.0f;
float BluefirstRotation = 0.0f;
float BluesecondRotation = 0.0f;
//WinMain
	...
	const char *vertexShaderSourceCode =
		...
		"{" \
			...
			"{" \
				...
				"output.Greenlight_dir = (float3)(GreenlightPosition - eyeCoordinate);" \
	...
	const char *pixelShaderSourceCode =
		...
		"float4 phong_ads_light;" \
			...
			"{" \
				...
				"float3 GreenNormalize_light_dir = normalize(Input.Greenlight_dir);" \
				...
				"float GreenTN_dot_ld = max(dot(Normalize_tnorm, GreenNormalize_light_dir),0.0f);" \
				...
				"float4 Greenambient = Greenla * ka;" \
				"float4 Greendiffuse = Greenld * kd * GreenTN_dot_ld;" \
				"float3 Greenreflection_vector = (float3)reflect(GreenNormalize_light_dir,Normalize_tnorm);" \
				"float4 Greenspecular = Greenls * ks * pow(max(dot(Greenreflection_vector , Normalize_viewer_vector ),0.0f),material_shinyness);" \
				"float4 Greenphong_ads_light = Greenambient + Greendiffuse + Greenspecular; " \
				...
				" phong_ads_light = Redphong_ads_light + Greenphong_ads_light + Bluephong_ads_light; " \
			"}" \
			"else" \
			"{" \
...
void display(void)
{
	...
	RedlightPosition[1] = RedsecondRotation;

	GreenlightPosition[0] = GreenfirstRotation;
	GreenlightPosition[2] = GreensecondRotation;

	BluelightPosition[1] = BluefirstRotation;
	BluelightPosition[2] = BluesecondRotation;
	...
	if (gblight == true)
	{
		...
		constantBuffer.GreenLa = XMVectorSet(GreenLightAmbient[0], GreenLightAmbient[1], GreenLightAmbient[2], GreenLightAmbient[3]);
		constantBuffer.GreenLd = XMVectorSet(GreenLightDiffuse[0], GreenLightDiffuse[1], GreenLightDiffuse[2], GreenLightDiffuse[3]);
		constantBuffer.GreenLs = XMVectorSet(GreenlightSpecular[0], GreenlightSpecular[1], GreenlightSpecular[2], GreenlightSpecular[3]);
		constantBuffer.GreenLightPositiion = XMVectorSet(GreenlightPosition[0], GreenlightPosition[1], GreenlightPosition[2], GreenlightPosition[3]);
	...
	*/
	...
	LightAnglezero = LightAnglezero - 0.002f;
	LightAngleone = LightAngleone + 0.002f;
	LightAngletwo = LightAngleone - 0.002f;
	RedfirstRotation = 270 * cos(LightAnglezero);
	RedsecondRotation = 270 * sin(LightAnglezero);
	GreenfirstRotation = 75 * cos(LightAngleone);
	GreensecondRotation = 75 * sin(LightAngleone);
