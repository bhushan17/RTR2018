...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Texture_Pyramid_BRK = NULL;
...
HRESULT initialize(void)
{
	...
	D3D11_BUFFER_DESC bufferDesc_PyramidTexture;
	ZeroMemory(&bufferDesc_PyramidTexture, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_PyramidTexture.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_PyramidTexture.ByteWidth = sizeof(float) * ARRAYSIZE(pyramid_texcoords);
	bufferDesc_PyramidTexture.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_PyramidTexture.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device_BRK->CreateBuffer(&bufferDesc_PyramidTexture, NULL, &gpID3D11Buffer_VertexBuffer_Texture_Pyramid_BRK);
	...
	gpID3D11DeviceContext_BRK->Map(gpID3D11Buffer_VertexBuffer_Texture_Pyramid_BRK, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, pyramid_texcoords, sizeof(pyramid_texcoords));
	gpID3D11DeviceContext_BRK->Unmap(gpID3D11Buffer_VertexBuffer_Texture_Pyramid_BRK, NULL);
	...
	{
		fopen_s(&gpFile_BRK, gszLogFileName, "a+");
		fprintf_s(gpFile_BRK, "LoadD3DTexture() Failed for Pyramid Texture. \n");
		fclose(gpFile_BRK);
		return(hr);
	...
	{
		fopen_s(&gpFile_BRK, gszLogFileName, "a+");
		fprintf_s(gpFile_BRK, "LoadD3DTexture() Succeeded for Pyramid Texture. \n");
		fclose(gpFile_BRK);
	}
	...
	hr = gpID3D11Device_BRK->CreateSamplerState(&samplerDesc, &gpID3D11SamplerState_Texture_Pyramid);
	if (FAILED(hr))
	{
		fopen_s(&gpFile_BRK, gszLogFileName, "a+");
		fprintf_s(gpFile_BRK, "ID3D11Device :: CreateSamplerState() Failed for Pyramid. \n");
		fclose(gpFile_BRK);
		return(hr);
	...
	{
		fopen_s(&gpFile_BRK, gszLogFileName, "a+");
		fprintf_s(gpFile_BRK, "ID3D11Device :: CreateSamplerState() Succeeded for Pyramid. \n");
		fclose(gpFile_BRK);
	}
...
void display(void)
{
	...
	offset = 0;
	gpID3D11DeviceContext_BRK->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Texture_Pyramid_BRK, &stride, &offset);

	//bind texture and sampler as pixel shader resorce
	gpID3D11DeviceContext_BRK->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_Texture_Pyramid);
	gpID3D11DeviceContext_BRK->PSSetSamplers(0, 1, &gpID3D11SamplerState_Texture_Pyramid);
