...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Color_Cube_BRK = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Texture_Cube_BRK = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Normal_Cube_BRK = NULL;
...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"};" \
		"vertex_output main(float4 pos : POSITION, float4 color : COLOR, float2 texcoord : TEXCOORD , float4 normal:NORMAL)" \
		"{" \
	...
	const char *pixelShaderSourceCode =
		...
		"float4 main(float4 pos:SV_POSITION,float4 color:COLOR, float2 texcoord:TEXCOORD, vertex_output Input) : SV_TARGET" \
		"{" \
		...
		"float3 Normalize_light_dir = normalize(Input.light_dir);" \
		"float3 Normalize_viewer_vector = normalize(Input.viewer_vector);" \
		"float TN_dot_ld = max(dot(Normalize_tnorm, Normalize_light_dir),0.0f);" \

		"float4 ambient = la * ka;" \
		"float4 diffuse = ld * kd * TN_dot_ld;" \
		"float3 reflection_vector = (float3)reflect(Normalize_light_dir,Normalize_tnorm);" \
		"float4 specular = ls * ks * pow(max(dot(reflection_vector , Normalize_viewer_vector ),0.0f),material_shinyness);" \
		...
		"float4 cubecolor =  phong_ads_light * myTexture2D.Sample(mySamplerState, texcoord)  *  color;" \
		"return(cubecolor);" \
		"}";
	...
	float cube_interlived[] =
	{
		//Side 1 (Top)               // Color             //texquard         //Normal
		//triangle 1
		...
		1.0f,1.0f,-1.0f,         0.0f,1.0f,1.0f,           0.0f,1.0f,        1.0f,0.0f,0.0f,
	...
	D3D11_BUFFER_DESC bufferDesc_CubeInterlived;
	ZeroMemory(&bufferDesc_CubeInterlived, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_CubeInterlived.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_CubeInterlived.ByteWidth = sizeof(float) * ARRAYSIZE(cube_interlived);
	bufferDesc_CubeInterlived.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_CubeInterlived.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device_BRK->CreateBuffer(&bufferDesc_CubeInterlived, NULL, &gpID3D11Buffer_VertexBuffer_Position_Cube_BRK);
	...
	/*D3D11_BUFFER_DESC bufferDesc_Cubecolor;
	ZeroMemory(&bufferDesc_Cubecolor, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_Cubecolor.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_Cubecolor.ByteWidth = sizeof(float) * ARRAYSIZE(cube_color);
	bufferDesc_Cubecolor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_Cubecolor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device_BRK->CreateBuffer(&bufferDesc_Cubecolor, NULL, &gpID3D11Buffer_VertexBuffer_Color_Cube_BRK);
	...
	{
		fopen_s(&gpFile_BRK, gszLogFileName, "a+");
		fprintf_s(gpFile_BRK, "ID3D11Device :: CreateBuffer() Failed for Rectangle texture Buffer. \n");
		fclose(gpFile_BRK);
		return(hr);
	...
	{
		fopen_s(&gpFile_BRK, gszLogFileName, "a+");
		fprintf_s(gpFile_BRK, "ID3D11Device :: CreateBufferShader() Succeeded For texture Buffer.  \n");
		fclose(gpFile_BRK);
	}
	*/
	...
	memcpy(mappedSubresource.pData, cube_interlived, sizeof(cube_interlived));
	gpID3D11DeviceContext_BRK->Unmap(gpID3D11Buffer_VertexBuffer_Position_Cube_BRK, NULL);
/*
	gpID3D11DeviceContext_BRK->Map(gpID3D11Buffer_VertexBuffer_Color_Cube_BRK, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, cube_color, sizeof(cube_color));
	gpID3D11DeviceContext_BRK->Unmap(gpID3D11Buffer_VertexBuffer_Color_Cube_BRK, NULL);
...
*/
	...
	inputElementDesc[3].Format = DXGI_FORMAT_R32G32B32_FLOAT; // u v 
	inputElementDesc[3].InputSlot = 3;		//Layout slot
	inputElementDesc[3].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA; //Class slot
	inputElementDesc[3].AlignedByteOffset = 0;  // D3D11_APPEND_ALIGNED_ELEMENT
...
HRESULT LoadD3DTexture(const wchar_t *textureFileName, ID3D11ShaderResourceView **ppID3D11ShaderResourceView)
{
	...
	{
		fopen_s(&gpFile_BRK, gszLogFileName, "a+");
		fprintf_s(gpFile_BRK, "CreateWICTextureFromFile() Failed for Testure Resource. \n");
		fclose(gpFile_BRK);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile_BRK, gszLogFileName, "a+");
		fprintf_s(gpFile_BRK, "CreateWICTextureFromFile() succeeded for texture resource. \n");
		fclose(gpFile_BRK);
...
void display(void)
{
	...
	gpID3D11DeviceContext_BRK->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Position_Cube_BRK, &stride, &offset);

	stride = sizeof(float) * 11;
	offset = sizeof(float) *  6;
	gpID3D11DeviceContext_BRK->IASetVertexBuffers(2, 1, &gpID3D11Buffer_VertexBuffer_Position_Cube_BRK, &stride, &offset);
	
	stride = sizeof(float) * 11;
	offset = sizeof(float) * 8;
	gpID3D11DeviceContext_BRK->IASetVertexBuffers(3, 1, &gpID3D11Buffer_VertexBuffer_Position_Cube_BRK, &stride, &offset);
	...
	translationMatrix = XMMatrixTranslation(0.0f, 0.0f, 9.0f);
	...
	XMMATRIX wvMatrix = scaleMatrix * rotationMatrix * rotationMatriX * rotationMatriZ* translationMatrix * worldMatrix  * gPerspectiveProjectionMatrix;
	...
	gpID3D11DeviceContext_BRK->Draw(6, 6);
	gpID3D11DeviceContext_BRK->Draw(6, 12);
	gpID3D11DeviceContext_BRK->Draw(6, 18);
	gpID3D11DeviceContext_BRK->Draw(6, 24);
	gpID3D11DeviceContext_BRK->Draw(6, 30);
