...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Color_Pyramid_BRK = NULL;
...
float gAnglePyramid = 0.0f; //Angle of rotation Pyramid
...
HRESULT initialize(void)
{
	...
	D3D11_BUFFER_DESC bufferDesc_Pyramidcolor;
	ZeroMemory(&bufferDesc_Pyramidcolor, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_Pyramidcolor.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_Pyramidcolor.ByteWidth = sizeof(float) * ARRAYSIZE(pyramid_color);
	bufferDesc_Pyramidcolor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_Pyramidcolor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device_BRK->CreateBuffer(&bufferDesc_Pyramidcolor, NULL, &gpID3D11Buffer_VertexBuffer_Color_Pyramid_BRK);
	...
	gpID3D11DeviceContext_BRK->Map(gpID3D11Buffer_VertexBuffer_Color_Pyramid_BRK, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, pyramid_color, sizeof(pyramid_color));
	gpID3D11DeviceContext_BRK->Unmap(gpID3D11Buffer_VertexBuffer_Color_Pyramid_BRK, NULL);
...
void display(void)
{
	...
	gpID3D11DeviceContext_BRK->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Color_Pyramid_BRK, &stride, &offset);
	...
	gpID3D11DeviceContext_BRK->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Color_Cube_BRK, &stride, &offset);
...
void unintialize(void)
{
	...
	if (gpID3D11Buffer_VertexBuffer_Position_Pyramid_BRK)
	{
		gpID3D11Buffer_VertexBuffer_Position_Pyramid_BRK->Release();
		gpID3D11Buffer_VertexBuffer_Position_Pyramid_BRK = NULL;
	}
