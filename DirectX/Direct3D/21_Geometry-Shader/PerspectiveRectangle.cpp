...
HRESULT initialize(void)
{
	...
	const char *geometryShaderSourceCode =
		...
		fopen_s(&gpFile_BRK, gszLogFileName, "a+");
		fprintf_s(gpFile_BRK, "Before Geometry Shader sourcecode.  \n");
		fclose(gpFile_BRK);
		
		ID3DBlob *pID3DBlob_GeometryShaderCode = NULL;
		ID3DBlob *pID3DBlob_GSError = NULL;
		...
		if(FAILED(hr))
		{
			if(pID3DBlob_GSError != NULL)
			{
				fopen_s(&gpFile_BRK,gszLogFileName,"a+");
				fprintf_s(gpFile_BRK, "D3DCompile() Failed For Geometry Shader : %s. \n", (char*)pID3DBlob_GSError->GetBufferPointer());
				fclose(gpFile_BRK);
				pID3DBlob_GSError->Release();
				pID3DBlob_GSError = NULL;
				return(hr);
			}
		}
		else
		{
			fopen_s(&gpFile_BRK,gszLogFileName,"a+");
			fprintf_s(gpFile_BRK, "D3DCompile() Succeeded For Geometry Shader \n");
			fclose(gpFile_BRK);
		}
		

		fopen_s(&gpFile_BRK, gszLogFileName, "a+");
		fprintf_s(gpFile_BRK, "After Geometry Shader sourcecode.  \n");
		fclose(gpFile_BRK);

		hr = gpID3D11Device_BRK->CreateGeometryShader(pID3DBlob_GeometryShaderCode->GetBufferPointer(), pID3DBlob_GeometryShaderCode->GetBufferSize(), NULL, &gpID3D11GeometryShader);
		
		if(FAILED(hr))
		{
			fopen_s(&gpFile_BRK,gszLogFileName,"a+");
			fprintf_s(gpFile_BRK, "gpID3D11Device::CreateGeometrySHader() Failed. \n");
			fclose(gpFile_BRK);
			return(hr);
		}
		else{
			fopen_s(&gpFile_BRK,gszLogFileName,"a+");
			fprintf_s(gpFile_BRK, "gpID3D11Device::CreateGeometrySHader() Sucseed. \n");
			fclose(gpFile_BRK);
		}

	gpID3D11DeviceContext_BRK->GSSetShader(gpID3D11GeometryShader,0,0);
	...
	gpID3D11DeviceContext_BRK->GSSetConstantBuffers(0, 1, &gpID3D11Buffer_ConstantBuffer_BRK);
