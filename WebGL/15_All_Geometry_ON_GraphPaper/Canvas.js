...
function draw()
{
	...
	for (index = 0; index < numpoints; index++)
	{
		...
		lineVertices[0] = (radius * Math.cos(angle));
		lineVertices[1] = (radius * Math.sin(angle));
		lineVertices[2] = 0;


		angle = (2 * Math.PI * (index+1)) / numpoints;
		lineVertices[3] = (radius * Math.cos(angle));
		lineVertices[4] = (radius * Math.sin(angle));
		lineVertices[5] = 0;
	...
	{
		angle = (2 * Math.PI *index) / numpoints;
		lineVertices[0] =  Math.cos(angle);
		lineVertices[1] = Math.sin(angle);
		lineVertices[2] = 0;


		angle = (2 * Math.PI * (index+1)) / numpoints;
		lineVertices[3] = Math.cos(angle);
		lineVertices[4] = Math.sin(angle);
		lineVertices[5] = 0;
	...
	var rectvertex = new Float32Array([
											-Math.sqrt(0.75),0.5,0.0,
											-Math.sqrt(0.75),-0.5,0.0,
											Math.sqrt(0.75),-0.5,0.0,
											Math.sqrt(0.75),0.5,0.0  
	...
	gl.bindBuffer(gl.ARRAY_BUFFER, vbo_line);
		gl.bufferData(gl.ARRAY_BUFFER, rectvertex, gl.DYNAMIC_DRAW);
		gl.bindBuffer(gl.ARRAY_BUFFER, null);
	
		gl.bindBuffer(gl.ARRAY_BUFFER, vbo_color_line);
		gl.bufferData(gl.ARRAY_BUFFER, reccolor, gl.DYNAMIC_DRAW);
		gl.bindBuffer(gl.ARRAY_BUFFER, null);
