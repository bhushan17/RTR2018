...
//extern float* objVertices;
//extern void modelloading();
...
extern void modelloading();
extern float *objVertices;
extern int **face_indicies;
extern float **model_vertices;
extern float **model_normals;
extern float **model_textures;
extern int faceIndiciesIndex;
...
void display()
{
	...
	modelViewMatrix = translate(0.0f, y, z);
	...
	/*for (int i = 0; i < faceIndiciesIndex-20; i++)
	{
		
			int vi = face_indicies[i][0];
			int ni = face_indicies[i][3];
			int ti = face_indicies[i][6];

			//glTexCoord2f(textures[ti][0], textures[ti][1]);
			//glNormal3f(normals[ni][0], normals[ni][1], normals[ni][2]);
			v[0] = model_vertices[vi-1][0];
			v[1] = model_vertices[vi-1][1];
			v[2]=  model_vertices[vi-1][2];
			v[3] = model_vertices[ni - 1][0];
			v[4] = model_vertices[ni - 1][1];
			v[5] = model_vertices[ni - 1][2];
			v[6] = model_vertices[ti - 1][0];
			v[7] = model_vertices[ti - 1][1];
			v[8] = model_vertices[ti - 1][2];
			glBindBuffer(GL_ARRAY_BUFFER, vbo_position);
		...
		glBindBuffer(GL_ARRAY_BUFFER, vbo_position);
		glBufferData(GL_ARRAY_BUFFER, faceIndiciesIndex * 9 * sizeof(float), objVertices, GL_DYNAMIC_DRAW);
		glBindBuffer(GL_ARRAY_BUFFER, 0);


		glDrawArrays(GL_TRIANGLES, 0, faceIndiciesIndex * 3);
	
	
/*	while (i < faceIndiciesIndex*9)
	{
		v[0] = objVertices[i++];
		v[1] = objVertices[i++];
		v[2] = objVertices[i++];
	...
	/*glLineWidth(2.0f);
	glDrawArrays(GL_LINE_LOOP, 0, numberOfVertex);*/
	/*glPointSize(5.0f);
	glDrawArrays(GL_POINTS, 0, numberOfVertex);*/
