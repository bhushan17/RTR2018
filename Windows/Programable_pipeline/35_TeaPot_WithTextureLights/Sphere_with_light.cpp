...
GLuint vao_cube;
GLuint vbo_position_cube, vbo_texture_cube, vbo_normal_cube;
...
int intialize(void)
{
	...
	const GLchar *FragmentShaderSourseCode =
		...
		"fragColor =  vec4(phong_ads_light,1.0) * texture(u_sampler, out_texquard) ; "
		"}";
...
void display()
{
	...
	for (int i = 0; i < sizeof(face_indicies) / sizeof(face_indicies[0]); i++)
	{
		...
		float texture_arr[6];
		texture_arr[0] = normals[ti][0];
		texture_arr[1] = normals[ti][1];
		...
		texture_arr[2] = normals[ti][0];
		texture_arr[3] = normals[ti][1];
		...
		texture_arr[4] = normals[ti][0];
		texture_arr[5] = normals[ti][1];
		...
		glBindBuffer(GL_ARRAY_BUFFER, vbo_texture_cube);
		glBufferData(GL_ARRAY_BUFFER, sizeof(texture_arr), texture_arr, GL_DYNAMIC_DRAW);
		glBindBuffer(GL_ARRAY_BUFFER, 0);
