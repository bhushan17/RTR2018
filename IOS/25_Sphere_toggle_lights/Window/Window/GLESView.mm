...
@implementation GLESView
{
  ...
  GLuint vertexShaderObject_PF_BRK;
  GLuint fragmentShaderObject_PF_BRK;
  GLuint shaderProgramObject_PF_BRK;
  ...
  GLuint modelMatrix_Uniform_pf_BRK, viewMatrix_Uniform_pf_BRK, projection_Uniform_pf_BRK;
  GLuint ld_Uniform_pf_BRK, la_Uniform_pf_BRK, ls_Uniform_pf_BRK, lightPosition_Uniform_pf_BRK;
  GLuint ka_Uniform_pf_BRK, kd_Uniform_pf_BRK, ks_Uniform_pf_BRK;
  GLuint LKeyPress_Uniform_pf_BRK, materialShinyness_Uniform_pf_BRK;
  bool toggle;
...
-(id)initWithFrame:(CGRect)frame;
{
	...
	if(self)
	{
        ...
        //*** VERTEX SHADER *****
         //create shader
         vertexShaderObject_PF_BRK=glCreateShader(GL_VERTEX_SHADER);
         ...
         glShaderSource(vertexShaderObject_PF_BRK, 1, (const GLchar**)&vertexShaderSourceCode, NULL);
         
         // compile shader
         glCompileShader(vertexShaderObject_PF_BRK);
         GLint iInfoLogLength = 0;
         GLint iShaderCompiledStatus = 0;
         char *szInfoLog = NULL;
         glGetShaderiv(vertexShaderObject_PF_BRK, GL_COMPILE_STATUS, &iShaderCompiledStatus);
         if(iShaderCompiledStatus == GL_FALSE)
         {
             glGetShaderiv(vertexShaderObject_PF_BRK, GL_INFO_LOG_LENGTH, &iInfoLogLength);
             if(iInfoLogLength > 0)
             {
                 ...
                 {
                     GLsizei written;
                     glGetShaderInfoLog(vertexShaderObject_PF_BRK, iInfoLogLength, &written, szInfoLog);
         ...
         //create shader
         fragmentShaderObject_PF_BRK = glCreateShader(GL_FRAGMENT_SHADER);
        ...
        "out vec4 fragColor;" \
         ...
         glShaderSource(fragmentShaderObject_PF_BRK,1,(const GLchar **)&fragmentShaderSourceCode,NULL);
         
         //compile shader
         glCompileShader(fragmentShaderObject_PF_BRK);
         glGetShaderiv(fragmentShaderObject_PF_BRK, GL_COMPILE_STATUS, &iShaderCompiledStatus);
         if(iShaderCompiledStatus == GL_FALSE)
         {
             glGetShaderiv(fragmentShaderObject_PF_BRK, GL_INFO_LOG_LENGTH, &iInfoLogLength);
             if(iInfoLogLength > 0)
             {
                 szInfoLog = (char*)malloc(iInfoLogLength);
                 if(szInfoLog!= NULL)
                 {
                     GLsizei written;
                     glGetShaderInfoLog(fragmentShaderObject_PF_BRK, iInfoLogLength, &written, szInfoLog);
         ...
         // **** Shader Program *****
         // create
         shaderProgramObject_PF_BRK = glCreateProgram();
         
         //attach vertex shader and fragment shader
         glAttachShader(shaderProgramObject_PF_BRK, vertexShaderObject_PF_BRK);
         
         //attach fragment shader
         glAttachShader(shaderProgramObject_PF_BRK, fragmentShaderObject_PF_BRK);
         
         //ore-link binding of shader program object with vertex shader position attribute
         glBindAttribLocation(shaderProgramObject_PF_BRK, VDG_ATTRIBUTE_VERTEX, "vPosition");
         glBindAttribLocation(shaderProgramObject_PF_BRK, VDG_ATTRIBUTE_NORMAL, "vNormal");
         
         
         //link shader
         glLinkProgram(shaderProgramObject_PF_BRK);
         GLint iShaderProgramLinkStatus = 0;
         glGetProgramiv(shaderProgramObject_PF_BRK, GL_LINK_STATUS, &iShaderProgramLinkStatus);
        // glGetShaderiv(fragmentShaderObject_BRK, GL_COMPILE_STATUS, &iShaderCompiledStatus);
         if(iShaderProgramLinkStatus == GL_FALSE)
         {
             glGetProgramiv(shaderProgramObject_PF_BRK, GL_INFO_LOG_LENGTH, &iInfoLogLength);
             if(iInfoLogLength > 0)
             {
                 szInfoLog = (char*)malloc(iInfoLogLength);
                 if(szInfoLog != NULL)
                 {
                     GLsizei written;
                     glGetProgramInfoLog(shaderProgramObject_PF_BRK, iInfoLogLength, &written, szInfoLog);
         ...
         modelMatrix_Uniform_pf_BRK = glGetUniformLocation(shaderProgramObject_PF_BRK, "u_model_matrix");
         viewMatrix_Uniform_pf_BRK = glGetUniformLocation(shaderProgramObject_PF_BRK, "u_view_matrix");
         projection_Uniform_pf_BRK = glGetUniformLocation(shaderProgramObject_PF_BRK, "u_projection_matrix");

         la_Uniform_pf_BRK = glGetUniformLocation(shaderProgramObject_PF_BRK, "u_la");
         ld_Uniform_pf_BRK = glGetUniformLocation(shaderProgramObject_PF_BRK, "u_ld");
         ls_Uniform_pf_BRK = glGetUniformLocation(shaderProgramObject_PF_BRK, "u_ls");
         lightPosition_Uniform_pf_BRK = glGetUniformLocation(shaderProgramObject_PF_BRK, "u_lightPosition");

         ka_Uniform_pf_BRK = glGetUniformLocation(shaderProgramObject_PF_BRK, "u_ka");
         kd_Uniform_pf_BRK = glGetUniformLocation(shaderProgramObject_PF_BRK, "u_kd");
         ks_Uniform_pf_BRK = glGetUniformLocation(shaderProgramObject_PF_BRK, "u_ks");
         materialShinyness_Uniform_pf_BRK = glGetUniformLocation(shaderProgramObject_PF_BRK, "u_materialShinyness");
         LKeyPress_Uniform_pf_BRK = glGetUniformLocation(shaderProgramObject_PF_BRK, "u_Lkeypress");
         ...
         glShaderSource(vertexShaderObject_BRK, 1, (const GLchar**)&vertexShaderSourceCode_PV, NULL);
         ...
         glShaderSource(fragmentShaderObject_BRK,1,(const GLchar **)&fragmentShaderSourceCode_PV,NULL);
...
-(void)drawView:(id)sender
{
   ...
   if(toggle == TRUE)
   {
       glUseProgram(shaderProgramObject_PF_BRK);
   }
   else{
   ...
   if(toggle == TRUE)
   {
       glUniformMatrix4fv(modelMatrix_Uniform_pf_BRK, 1, GL_FALSE, modelMatrix);
       glUniformMatrix4fv(viewMatrix_Uniform_pf_BRK, 1, GL_FALSE, viewMatrix);
       glUniformMatrix4fv(projection_Uniform_pf_BRK, 1, GL_FALSE, perspectiveProjectionMatrix_BRK);
   }
   else
   {
   ...
   if (bLight_BRK == TRUE)
   {
       ...
       glUniform1i(LKeyPress_Uniform_pf_BRK, 1);
       glUniform3fv(la_Uniform_pf_BRK, 1 , lightAmbient );
       glUniform3fv(ld_Uniform_pf_BRK, 1, lightDiffuse);
       glUniform3fv(ls_Uniform_pf_BRK, 1, lightSpecular);
       glUniform4fv(lightPosition_Uniform_pf_BRK, 1, lightPosition);

       glUniform3fv(ka_Uniform_pf_BRK, 1, materialAmbient);
       glUniform3fv(kd_Uniform_pf_BRK, 1, materialDiffuse);
       glUniform3fv(ks_Uniform_pf_BRK, 1, materialSpecular);
       glUniform1f(materialShinyness_Uniform_pf_BRK, materialShinyness);
   }
   else
   {
       glUniform1i(LKeyPress_Uniform_BRK, 0);
       glUniform1i(LKeyPress_Uniform_pf_BRK, 0);
   }
   //****bind vao_Sphere_BRK ***
   glBindVertexArray(vao_Sphere_BRK);
   glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_sphere_element_BRK);
