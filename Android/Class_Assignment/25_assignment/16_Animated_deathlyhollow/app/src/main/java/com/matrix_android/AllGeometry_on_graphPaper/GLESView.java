package com.matrix_android.Animated_deathlyhollow;
...
public class GLESView extends GLSurfaceView implements  OnGestureListener, OnDoubleTapListener,  GLSurfaceView.Renderer
{
	...
	private float lineycoordinate = 1.0f;
		
		private float triangletranslatex = 1.0f;

		private float triangletranslate = 0.0f;
		private float circletranslate = 0.0f;
		private float linetranslate = 0.0f;

		
		private float trianglerotationangle = 0.0f;
		private float circlerotationangle = 0.0f;
	...
	private void display()
	{
		...
		float incentery = (1 - (float)Math.sqrt(5.0f)) / (float)(1 + Math.sqrt(5.0f));
		...
		Matrix.translateM(modelViewMatrix,0,1 - triangletranslate, -1 + triangletranslate, -6 * triangletranslate);
						
		Matrix.setRotateM(rotateMatrix,0,trianglerotationangle, 0.0f, 1.0f, 0.0f);				
		...
		if (triangletranslate < 0.7f) {
			triangletranslate = triangletranslate + 0.01f;
		}
		...
		if (trianglestatus == 1)
		{
			if (circletranslate < 0.7f) {
				circletranslate = circletranslate + 0.01f;
			...
			//Do necessary transformation
			Matrix.translateM(modelViewMatrix,0,-1 + circletranslate, -1 + (incentery + 1) * circletranslate, (float)-6.0 * circletranslate);
							
			Matrix.setRotateM(rotateMatrix,0,circlerotationangle, 0.0f, 1.0f, 0.0f);				
			//Do necessary Matrix multiplication
			Matrix.multiplyMM(modelViewMatrix,0,modelViewMatrix,0,rotateMatrix,0);
			Matrix.multiplyMM(modelViewProjectionMatrix,0,perspectiveProjection,0,modelViewMatrix,0);
			...
			float radius = (2 / (1 + (float)Math.sqrt(5.0f)));
		...
		if (circlestatus== 1) {
			...
			if (linetranslate < 1.0f) {
				linetranslate = linetranslate + 0.01f;
				lineycoordinate = lineycoordinate - 0.02f;
			}
			else {
				linetranslate = 1.0f;
		...
		//Intialize above matrix to Identity;
			...
			//Do necessary transformation
			Matrix.translateM(modelViewMatrix,0,0.0f, 1 - linetranslate, -6.0f * linetranslate);
			...
			lineVertices[2] = 0.0f;
			lineVertices[3] = 0.0f;
			lineVertices[4] = lineycoordinate;
			lineVertices[5] = 0.0f;
		...
		if (trianglerotationangle > 360)
		{
			...
			trianglerotationangle = trianglerotationangle + 10.0f;
			circlerotationangle = circlerotationangle + 10.0f;
		}
