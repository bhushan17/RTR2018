package com.matrix_android.Sphere_toggle_lights;
...
public class GLESView extends GLSurfaceView implements  OnGestureListener, OnDoubleTapListener,  GLSurfaceView.Renderer
{
	...
	private int modelMatrix_Uniform_perVertex, viewMatrix_Uniform_perVertex, projection_Uniform_perVertex;
	private int ld_Uniform_perVertex, la_Uniform_perVertex, ls_Uniform_perVertex, lightPosition_Uniform_perVertex;
	private int ka_Uniform_perVertex, kd_Uniform_perVertex, ks_Uniform_perVertex;
	private int LKeyPress_Uniform_perVertex, materialShinyness_Uniform_perVertex;
	...
	private void intialize()
	{
		...
		final String vertexShaderSourceCode_perVertex = String.format
		("#version 320 es" +
		"\n" +
		"precision lowp int;"+
		"precision highp float;"+
		"in vec4 vPosition;" +
		"in vec3 vNormal;" +
		"uniform mat4 u_model_matrix_perVertex;" +
		"uniform mat4 u_view_matrix_perVertex;" +
		"uniform mat4 u_projection_matrix_perVertex;" +
		"uniform int u_Lkeypress_perVertex;" +
		"uniform vec3 u_la_perVertex;" +
		"uniform vec3 u_ld_perVertex;" +
		"uniform vec3 u_ls_perVertex;" +
		"uniform vec4 u_lightPosition_perVertex;" +
		"uniform vec3 u_ka_perVertex;" +
		"uniform vec3 u_kd_perVertex;" +
		"uniform vec3 u_ks_perVertex;" +
		"uniform float u_materialShinyness_perVertex;" +
		"out vec3 phong_ads_light;" +
		"void main(void)" +
		"{" +
		"if(u_Lkeypress_perVertex==1)" +
		"{" +
		"vec4 eye_coordinate = u_view_matrix_perVertex * u_model_matrix_perVertex * vPosition;" +
		"vec3 Transformednormal=normalize(mat3(u_view_matrix_perVertex * u_model_matrix_perVertex) * vNormal);" +
		"vec3 lightDirection =normalize(vec3(u_lightPosition_perVertex - eye_coordinate));" +
		...
		"vec3 ambient = u_la_perVertex * u_ka_perVertex;" +
		"vec3 diffuse = u_ld_perVertex * u_kd_perVertex * TN_dot_ld;" +
		"vec3 specular = u_ls_perVertex * u_ks_perVertex * pow(max(dot(reflection_vector , viewer_vector ),0.0f),u_materialShinyness_perVertex);" +
		"phong_ads_light = ambient + diffuse + specular;" +
		"}" +
		"else" +
		"{" +
		"phong_ads_light = vec3(1.0f,1.0f,1.0f);" +
		"}" +
		"gl_Position = u_projection_matrix_perVertex * u_view_matrix_perVertex * u_model_matrix_perVertex * vPosition;" +
		"}");
		
		GLES32.glShaderSource(vertexShaderObject_perVertex,vertexShaderSourceCode_perVertex);
		GLES32.glCompileShader(vertexShaderObject_perVertex);
		
		int[]ishaderCompileStatus_perVertex = new int[1];
		int[] iInfolog_perVertex = new int[1];
		
		SzInfolog = null;
		GLES32.glGetShaderiv(vertexShaderObject_perVertex, GLES32.GL_COMPILE_STATUS, ishaderCompileStatus_perVertex, 0);
		if(ishaderCompileStatus_perVertex[0] == 0)
		{
			GLES32.glGetShaderiv(vertexShaderObject_perVertex, GLES32.GL_INFO_LOG_LENGTH, iInfolog_perVertex, 0);
			if(iInfolog_perVertex[0]>0)
			{
		...
		final String FragmentShaderSourseCode_perVertex = String.format
		("#version 320 es" +
		"\n" +
		"precision highp float;"+
		"\n" +
		...
		GLES32.glShaderSource(fragmentShaderObject_perVertex,FragmentShaderSourseCode_perVertex);
		GLES32.glCompileShader(fragmentShaderObject_perVertex);
		
		ishaderCompileStatus_perVertex[0] = 0;
		iInfolog_perVertex[0] = 0;
		
		SzInfolog = null;
		GLES32.glGetShaderiv(fragmentShaderObject_perVertex, GLES32.GL_COMPILE_STATUS, ishaderCompileStatus_perVertex, 0);
		if(ishaderCompileStatus_perVertex[0] == 0)
		{
			GLES32.glGetShaderiv(fragmentShaderObject_perVertex, GLES32.GL_INFO_LOG_LENGTH, iInfolog_perVertex, 0);
			if(iInfolog_perVertex[0]>0)
			{
		...
		int[]iProgramLinkStatus_perVertex = new int[1];
		iInfolog_perVertex[0] = 0;
		
		SzInfolog = null;
		
		//ErrorCheking
		GLES32.glGetProgramiv(shaderProgramObject_perVertex,GLES32.GL_LINK_STATUS,iProgramLinkStatus_perVertex,0);
		if (iProgramLinkStatus_perVertex[0] == 0)
		{
			GLES32.glGetProgramiv(shaderProgramObject_perVertex, GLES32.GL_INFO_LOG_LENGTH, iInfolog_perVertex,0);
			if(iInfolog_perVertex[0]>0)
			{
		...
		//Post linking retriving/getting uniform location 
		modelMatrix_Uniform_perVertex = GLES32.glGetUniformLocation(shaderProgramObject_perVertex, "u_model_matrix_perVertex");
		viewMatrix_Uniform_perVertex = GLES32.glGetUniformLocation(shaderProgramObject_perVertex, "u_view_matrix_perVertex");
		projection_Uniform_perVertex = GLES32.glGetUniformLocation(shaderProgramObject_perVertex, "u_projection_matrix_perVertex");
		
		la_Uniform_perVertex = GLES32.glGetUniformLocation(shaderProgramObject_perVertex, "u_la_perVertex");
		ld_Uniform_perVertex = GLES32.glGetUniformLocation(shaderProgramObject_perVertex, "u_ld_perVertex");
		ls_Uniform_perVertex = GLES32.glGetUniformLocation(shaderProgramObject_perVertex, "u_ls_perVertex");
		lightPosition_Uniform_perVertex = GLES32.glGetUniformLocation(shaderProgramObject_perVertex, "u_lightPosition_perVertex");
		
		ka_Uniform_perVertex = GLES32.glGetUniformLocation(shaderProgramObject_perVertex, "u_ka_perVertex");
		kd_Uniform_perVertex = GLES32.glGetUniformLocation(shaderProgramObject_perVertex, "u_kd_perVertex");
		ks_Uniform_perVertex = GLES32.glGetUniformLocation(shaderProgramObject_perVertex, "u_ks_perVertex");
		
		
		materialShinyness_Uniform_perVertex = GLES32.glGetUniformLocation(shaderProgramObject_perVertex, "u_materialShinyness_perVertex");
		LKeyPress_Uniform_perVertex = GLES32.glGetUniformLocation(shaderProgramObject_perVertex, "u_Lkeypress_perVertex");
	...
	private void display()
	{
		...
		if(true==flag)
		{
			GLES32.glUniformMatrix4fv(modelMatrix_Uniform_perVertex, 1, false, modelMatrix,0);
			GLES32.glUniformMatrix4fv(viewMatrix_Uniform_perVertex, 1, false, viewMatrix,0);
			GLES32.glUniformMatrix4fv(projection_Uniform_perVertex, 1, false, perspectiveProjectionMatrix,0);
		}
		else
		{
		...
		if (bLight == true)
		{
			...
			if(true==flag)
			{
			GLES32.glUniform1i(LKeyPress_Uniform_perVertex, 1);
			...
			GLES32.glUniform1f(materialShinyness_Uniform_perVertex, materialShinyness);
			}
			else{
		...
		else
		{
			...
			if(true==flag)
			{
				GLES32.glUniform1i(LKeyPress_Uniform_perVertex, 0);
			}
			else
			{
