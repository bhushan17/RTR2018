...
GLuint vbo;
GLuint mvpUniform;
mat4 aurthographicProjectionMatrix;  //this is from Vmath
...
void intialize(void)
{
	...
	aurthographicProjectionMatrix = mat4::identity();
...
void resize(int width, int height){
	...
	if (width < height)
	{
		aurthographicProjectionMatrix = ortho(-100.0, 100.0, -100.0 * (height / width), 100.0 *(height / width), -100.0, 100.0);
	}
	else {
		aurthographicProjectionMatrix = ortho(-100.0*(width / height), 100.0* (width / height), -100.0 , 100.0 , -100.0, 100.0);
	}
...
void display(void){
	...
	modelViewProjectionMatrix = aurthographicProjectionMatrix * modelViewMatrix;
