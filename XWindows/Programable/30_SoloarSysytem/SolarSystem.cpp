...
float lightAmbient[4] = { 1.0f,1.0f,1.0f,0.0f };
float lightDiffuse[4] = { 1.0f, 0.5f, 0.05f,1.0f };
float lightDiffuse2[4] = {  0.1f, 0.1f, 0.5f,1.0f };
float lightDiffuse3[4] = { 0.7f, 0.7f, 0.7f,1.0f };
...
void display(void){
	...
	BRKStackPush(modelViewMatrix);

	//Send necessary Matrix to shader in respective uniform
	glUniformMatrix4fv(modelMatrix_Uniform, 1, GL_FALSE, modelViewMatrix);
	...
	*/
	...
	glBindVertexArray(0);
	//BRKPopMatrix(&modelViewMatrix);
	BRKStackPush(modelViewMatrix);

	rotationMatrix = rotate(year, 0.0f, 1.0f, 0.0f);
	translateMatrix = translate(1.3f, 0.0f, 0.0f);
	scaleMatrix = scale(0.5f, 0.5f, 0.5f);
	modelViewMatrix = modelViewMatrix * rotationMatrix * translateMatrix * scaleMatrix;

	BRKStackPush(modelViewMatrix);
	
	rotationMatrix = rotate(90.0f, 1.0f, 0.0f, 0.0f);
	rotationMatrix = rotate(day, 0.0f, 1.0f, 0.0f);
	...
	BRKPopMatrix(&modelViewMatrix);
	rotationMatrix = rotate(moon, 0.0f, 0.0f, 1.0f);
	translateMatrix = translate(1.0f, 0.0f, 0.0f);
	rotationMatrix = rotate(90.0f, 0.0f, 1.0f, 0.0f);
	rotationMatrix = rotate(moonself, 0.0f, 1.0f, 0.0f);
