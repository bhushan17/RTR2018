...
GLuint vbo_position_pyramid, vbo_position_cube, vbo_texture_pyramid, vbo_texture_cube;
GLuint texturePyramid, textureCube, samplerUniform;
GLuint mvpUniform;
...
void CreateWindow(void)
{
	...
	XStoreName(gpDisplay, gWindow, "Texture XWindow");
...
void intialize(void)
{
	...
	loadTexture(&texturePyramid, "Stone.bmp");
	loadTexture(&textureCube, "Vijay_Kundali.bmp");
...
void display(void){
	...
	modelViewMatrix = mat4::identity();
	translateMatrix = mat4::identity();
	scaledMatrix = mat4::identity();
	rotationMatrix = mat4::identity();
	modelViewProjectionMatrix = mat4::identity();

	//Do necessary Matrix multiplication
	translateMatrix = translate(2.0f, 0.0f, -6.0f);
	scaledMatrix = scale(0.75f,0.75f,0.75f);
	rotationMatrix = rotate(angle_cube, angle_cube, angle_cube);
	//rotationMatrix = rotate(angle_cube, 1.0f, 0.0f, 0.0f);
	translateMatrix = translateMatrix * scaledMatrix;
	translateMatrix = translateMatrix * rotationMatrix;
