...
GLfloat angle_triangle = 0.0f, angle_rectangle = 0.0f;
...
void update (void)
{
	if (angle_triangle > 360)
	{
		...
		angle_rectangle = angle_rectangle + 0.05f;
	}
}
...
void display(void){
	...
	//Do necessary Matrix multiplication
	translateMatrix = translate(2.0f, 0.0f, -6.0f);
	rotationMatrix = rotate(angle_rectangle, 1.0f, 0.0f, 0.0f);
	...
	glUseProgram(0);	glXSwapBuffers(gpDisplay, gWindow);
	}
