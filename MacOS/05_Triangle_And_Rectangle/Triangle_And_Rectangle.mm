...
@private
	...
	GLuint vboTriangle_BRK;
	GLuint vaoRectangle_BRK;
	GLuint vboRectangle_BRK;
	GLuint mvpUniform;
...
-(void)prepareOpenGL
{
	...
	// *** vertices, colors, shader attribs, vboTriangle_BRK, vaoTriangle_BRK intiallize ****
	...
	glGenBuffers(1, &vboTriangle_BRK);
	glBindBuffer(GL_ARRAY_BUFFER, vboTriangle_BRK);
	...
	glGenBuffers(1, &vboRectangle_BRK);
	glBindBuffer(GL_ARRAY_BUFFER, vboRectangle_BRK);
...
-(void)dealloc
{
	...
	//destroy vboTriangle_BRK
	if(vboTriangle_BRK)
	{
		glDeleteBuffers(1,&vboTriangle_BRK);
		vboTriangle_BRK = 0;
	}
